...
class OpenMatchTrainerProbe:
    ...
    def bbox_stat(self, unlabel_gt, unlabel_pseudo, name, bbox_stat_list):
        ...
        for metric in bbox_stat_list:
            stats_per, sum_gpu_names_per = getattr(self, metric)(
                unlabel_gt, unlabel_pseudo, name
            )
    ...
    def compute_fp_gtoutlier(self, unlabel_gt, unlabel_pseudo, name):
        ...
        if len(unlabel_gt) != 0:
            for gt, pseudo in zip(unlabel_gt, unlabel_pseudo):
                ...
                if len(gt) != 0 and len(pseudo) != 0:
                    max_iou, max_idx = pairwise_iou(
                        gt.gt_boxes.to("cuda"), pp_boxes
                    ).max(1)
                    ood_idx = gt.gt_classes == -1

                    num_gt_ood_object += ood_idx.sum().item()
                    num_gt_fp_ood_object += (
                        (max_iou[ood_idx] > self.BOX_AP).sum().item()
                    )
            ...
            results = {
                "Analysis_" + name + "/num_gt_ood_object": num_gt_ood_object,
    ...
    def compute_num_box(self, unlabel_gt, unlabel_pseudo, name, processed=False):
        ...
        if num_valid_img > 0:
            ...
            box_probe["Analysis_" + name + "/Num_bbox_inlier"] = (
                num_bbox_in / num_valid_img
            )
            box_probe["Analysis_" + name + "/Num_bbox_outlier"] = (
                num_bbox_out / num_valid_img
            )

            if name != "gt":  # prediciton, background number
                box_probe["Analysis_" + name + "/Conf"] = avg_conf / num_valid_img
                box_probe["Analysis_" + name + "/Num_bbox_background"] = (
                    num_bg / num_valid_img
                )
                box_probe["Analysis_" + name + "/background_fp_ratio"] = (
                    num_bg / num_bbox
                )
                box_probe["Analysis_" + name + "/background_tp_ratio"] = (
                    num_bbox_in / num_bbox
                )
        else:
                ...
                if len(gt) != 0 and len(pred) != 0:
                    ...
                    if find_fg_mask.sum() > 0:
                        ...
                        results["recall_outlier_fg_nume"] += (
                            pred_outlier[gt_outlier] == gt_outlier[gt_outlier]
                        ).sum()
